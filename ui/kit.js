...
const CSS = `
.bio-main { container-type: inline-size; }
.bio-row3 { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: var(--sy-s3); }
.bio-row2 { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: var(--sy-s3); }
.bio-ask { display: grid; grid-template-columns: minmax(0, 1fr) minmax(260px, 320px); grid-template-rows: minmax(0, 1fr); gap: var(--sy-s3); }
.bio-item { display: grid; grid-template-columns: minmax(0, 1fr) minmax(300px, 380px); gap: var(--sy-s3); align-items: start; }
@container (max-width: 620px) {
  .bio-ask { grid-template-columns: minmax(0, 1fr); grid-template-rows: none; height: auto !important; }
}
@container (max-width: 700px) {
  .bio-row3 { grid-template-columns: minmax(0, 1fr); }
  .bio-item { grid-template-columns: minmax(0, 1fr); }
}
@container (max-width: 640px) {
  .bio-row2 { grid-template-columns: minmax(0, 1fr); }
}
`;

/** The plugin's own layout rules, added to the document the first time a panel is drawn. */
export function ensureStyles() {
  if (typeof document === 'undefined' || document.getElementById('bio-kit-css')) return;
